"""
Based on on work of (c)2023 David Horton.
...
from typing import BinaryIO, Literal, Callable, Awaitable
from os import stat_result
import logging
import socket
import ssl
import asyncio
from asyncio import open_connection, start_server, sleep
from time import localtime, time
from pathlib import PurePosixPath as FTPPath, Path
...
class Session:
    ...
    def __init__(
        ...
        client_ip: str,
        client_port: str,
        ctrl_reader: asyncio.StreamReader,
        ...
        # self.uid = 65534
        self.cwd = FTPPath("/")  # virtual user's path
    ...
    def listdir(self, userpath: str | None) -> tuple[list[Path], FTPPath]:
        ftp_path, real_path = self.decode_path(userpath)
        return list(real_path.iterdir()), ftp_path

    def stat(self, userpath: str | None) -> tuple[stat_result, FTPPath]:
        ftp_path, real_path = self.decode_path(userpath)
        return real_path.stat(), ftp_path

    def decode_path(
        self: Session, path: str | None, root: FTPPath = FTPPath("/")
    ) -> tuple[FTPPath, Path]:
        if path is None or path.startswith("-"):
            # client sent a command-line option, do nothing
            return self.cwd, FTPS_ROOT / self.cwd.relative_to(root)
        try:
            ftp_path = (self.cwd / FTPPath(path)).relative_to(root)
        except Exception:
            breakpoint()
        if not ftp_path.is_relative_to(root):
            return FTPPath("/"), FTPS_ROOT
        return ftp_path, FTPS_ROOT / ftp_path.relative_to(root)
...
class FTPS:
    ...
    _ftp_cmd_dict: dict[
        str,
        Callable[[Session, str | None], Awaitable[bool]],
    ]
    ...
    def __init__(
        ...
        port: int,
        ssl: ssl.SSLContext,
        server_name: str = "FTPdLite (MicroPython)",
        ipv4_pasv_forced_response_address: str = "127.0.0.1",
        pasv_port_range: range = range(49152, 49407),
    ):
        self._host = host
        self._port = port
        self._ssl = ssl
        self._server_name = server_name
        self._ipv4_pasv_forced_response_address = (
    ...
    # @staticmethod
    # def decode_path(
    #     session: Session, path: str | None, root: FTPPath = FTPPath("/")
    # ) -> FTPPath:
    #     """
    ...
    #         # client sent a command-line option, do nothing
    #         return session.cwd
    #     ftp_path = session.cwd / FTPPath(path)
    #     if not ftp_path.is_relative_to(root):
    #         return FTPPath("/")
    #     return ftp_path
    # if path.startswith("/"):
    ...
    @staticmethod
    def read_file_chunk(file: BinaryIO):
        """
        ...
        while True:
            chunk = file.read(
                512
            )  # small chunks to avoid out of memory errors
            if chunk:
                yield chunk
    ...
    async def find_session(self, search_value: str):
        """
    ...
    def parse_request(self, req_buffer: bytes) -> tuple[str, str | None]:
        """
    ...
    async def send_response(
        self, session: Session, code: int, msg: str | list[str] = ""
    ) -> bool:
        """
    ...
    # async def mode(self, session: Session, param: str | None) -> bool:
    #     """
    ...
    #             session, 504, "Transfer mode not supported."
    #         )
    #     return True

    async def noop(self, session: Session, _: str | None) -> bool:
        """
    ...
    async def passwd(self, session: Session, password: str | None) -> bool:
        """
        ...
        #         uid = i
        #         self.debug(
        #             f"Found user account for: {session.username} (uid={uid})"
        #         )
        #         break
        # if uid is not None:
        ...
        #         self.debug(
        #             f"Authenticating against hashed password: {stored_password}"
        #         )
        #         authenticated = SHA256AES.verify_passwd_entry(
        #             stored_password, password
        #         )
        #     else:
        #         self.debug(
        #             "Authenticating against cleartext password: ********"
        #         )
        #         authenticated = stored_password == password
        session.authenticated = authenticated = session.username == password
        if not authenticated:
            await sleep(1.0)  # throttle repeated bad attempts
            await self.send_response(
                session, 430, "Invalid username or password."
            )
        else:
            await self.send_response(
    ...
    async def pasv(self, session: Session, _: str | None) -> bool:
        """
        ...
        Returns:
            boolean: True
        """
        host_octets = self._ipv4_pasv_forced_response_address.replace(".", ",")
        ...
        session.data_listener = await start_server(
            self.on_data_connect, self._host, port, limit=1, ssl=self._ssl
        )
        await self.send_response(
            session,
            227,
            "Entering passive mode. "
            f"({host_octets},{port_octet_high},{port_octet_low})",
        )
        return True

    async def port(
        self, session: Session, address: str | None
    ) -> Literal[True]:
        """
        ...
        if address.count(",") != 5:
            ...
            try:
                ...
                await self.send_response(
                    session, 200, "PORT command successful."
                )
        return True

    async def quit(self, session: Session, _: str | None) -> Literal[False]:
        """
    ...
    async def retr(
        self, session: Session, filepath: str | None
    ) -> Literal[True]:
        """
        ...
        if not filepath:
            ...
            filepath = FTPS.decode_path(session, filepath)
            try:
                stat(filepath)
            except OSError:
                await self.send_response(session, 550, FTPS.ENOENT)
            else:
                if await self.verify_data_connection(session) is False:
                    ...
                    await self.send_response(
                        session, 150, "Transferring file."
                    )
                    try:
                        with open(filepath, "rb") as file:
                            for chunk in FTPS.read_file_chunk(file):
                                session.data_writer.write(chunk)
                                await session.data_writer.drain()
                    except OSError:
                        await self.send_response(
                            session, 451, "Error reading file."
                        )
                    else:
                        await self.send_response(
                            session, 226, "Transfer finished."
                        )
                        await self.close_data_connection(session)
        return True
    ...
    async def stor(
        ...
        if not filepath:
            ...
            if session.has_write_access(filepath) is False:
                await self.send_response(session, 550, FTPS.EACCES)
            else:
                if await self.verify_data_connection(session) is False:
                    await self.send_response(
                        session,
                    ...
                    try:
                        ...
                        await self.send_response(
                            session, 451, "Error writing file."
                        )
                    else:
                        await self.send_response(
                            session, 226, "Transfer finished."
    ...
    async def stru(self, session: Session, param: str | None) -> Literal[True]:
        """
        ...
        Returns: True
        """
        if param and param.upper() == "F":
            await self.send_response(session, 200, "OK.")
        else:
            await self.send_response(session, 504, "Structure not supported.")
        return True

    async def type(self, session: Session, type: str | None) -> Literal[True]:
        """
        ...
        """
        if type and type.upper() in ("A", "A N", "I", "L 8"):
    ...
    async def user(
        self, session: Session, username: str | None
    ) -> Literal[True]:
        """
        ...
        # if self._accounts != []:
        await self.send_response(
            session, 331, f"Password required for {username}."
        )
        # else:
        #     await self.send_response(
        #         session, 230, f"User {username} logged in."
        #     )
        #     try:
    ...
    async def verify_data_connection(self, session: Session) -> bool:
        """
        ...
        try:
            ...
            await sleep(0.2)  # if not, wait and try again
        try:
            session.data_reader
            session.data_writer
        except AttributeError:
    ...
    async def close_data_connection(self, session: Session) -> None:
        """
    ...
    async def close_ctrl_connection(self, session: Session) -> None:
        """
    ...
    #     if now < jan_1_2023:
    #         print(
    #             "WARNING: System clock not set. File timestamps will be incorrect."
    #         )
    #     if loop is None:
    #         loop = get_event_loop()
    ...
    async def create_server(self) -> asyncio.Server:
        """
        `await server.serve_forever()` on returned server
        """
        server = await asyncio.start_server(
            self.on_ctrl_connect,
            self._host,
            self._port,
            ssl=self._ssl,
        )
    ...
    @staticmethod
    def date_format(timestamp: float) -> str:
        """
    ...
    @staticmethod
    def human_readable(byte_size: int) -> str:
    ...
    async def cdup(self, session: Session, _: str | None) -> Literal[True]:
        """
    ...
    async def cwd(
        self, session: Session, userpath: str | None
    ) -> Literal[True]:
        """
        ...
        Args:
            session (object): the FTP client's login session info
            userpath (string): a path indicating a directory resource

        Returns:
            boolean: True
        """
        # dirpath = FTPS.decode_path(session, dirpath)
        try:
            properties, dirpath = session.stat(userpath)
        except OSError:
            await self.send_response(session, 550, FTPS.ENOENT)
        else:
    ...
    async def epsv(self, session: Session, _: str | None) -> Literal[True]:
        """
        Start a new data listener on one of the high numbered ports and
    ...
    async def feat(self, session: Session, _: str | None) -> Literal[True]:
        """
    ...
    async def help(self, session: Session, _: str | None) -> Literal[True]:
        """
    ...
    async def list(
        ...
        Returns:
            boolean: True
        """
        # dirpath = FTPS.decode_path(session, userpath)
        try:
            dir_entries, dirpath = session.listdir(userpath)
        except OSError:
            ...
            if await self.verify_data_connection(session) is False:
                ...
                await self.send_response(
                    session, 150, f"Contents of: {dirpath}"
                )
                for entry in dir_entries:
                    self.debug(f"Fetching properties of: {entry}")
                    ...
                    if properties[0] & 0x4000:  # entry is a directory
                        ...
                        permissions = "-rw-rw-r--"
                        size = FTPS.human_readable(properties[6])
                    uid = "root" if properties[4] == 0 else properties[4]
                    gid = "root" if properties[5] == 0 else properties[5]
                    mtime = FTPS.date_format(properties[8])
                    formatted_entry = f"{permissions}  1  {uid:4}  {gid:4}  {size:>10s}  {mtime:>11s}  {entry.name}"
    ...
    async def mkd(
        self, session: Session, dirpath: str | None
    ) -> Literal[True]:
        """
    ...
    async def mode(self, session: Session, param: str | None) -> Literal[True]:
        """
        This server (and most others) only supports stream mode. RFC-959
        ...
        try:
            ...
            if await self.verify_data_connection(session) is False:
                ...
                try:
                    ...
                    await session.data_writer.drain()
                    await self.send_response(
                        session, 226, "Directory list sent."
                    )
                    await self.close_data_connection(session)
        return True

    async def opts(
        self, session: Session, option: str | None
    ) -> Literal[True]:
        """
    ...
    async def rnfr(
        self, session: Session, rnfr_path: str | None
    ) -> Literal[True]:
        """
        ...
        if not rnfr_path:
            ...
            rnfr_path = FTPS.decode_path(session, rnfr_path)
            try:
                stat(rnfr_path)
            except OSError:
                await self.send_response(session, 550, FTPS.ENOENT)
    ...
    async def rnto(
        self, session: Session, rnto_path: str | None
    ) -> Literal[True]:
        """
        ...
        if not rnto_path:
            ...
            rnto_path = FTPS.decode_path(session, rnto_path)
    ...
    # async def site(self, session: Session, cmdline: str) -> Literal[True]:
    #     """
    ...
    async def stat(
        self, session: Session, pathname: str | None
    ) -> Literal[True]:
        """
    ...
    async def syst(self, session: Session, _: str | None) -> Literal[True]:
        """
    ...
    # async def site_df(self, session: Session, filesystem):
    ...
    #         size_hr = FTPS.human_readable(size)
    #         avail = int(blocks_available * fragment_size)
    #         avail_hr = FTPS.human_readable(avail)
    #         used = size - avail
    #         used_hr = FTPS.human_readable(used)
    ...
    # async def site_free(self, session: Session, _):
    #     free = FTPS.human_readable(mem_free())
    #     used = FTPS.human_readable(mem_alloc())
    #     total = FTPS.human_readable(mem_free() + mem_alloc())
    #     output = [
    #         "         Total       Used      Avail",
    ...
    # async def site_gc(self, session: Session, _):
    #     before = mem_free()
    #     gc_collect()
    #     after = mem_free()
    #     regained = FTPS.human_readable(after - before)
    #     return 211, f"Additional {regained} available."

    # async def site_hashpass(self, session: Session, cleartext):
    #     if not cleartext:
    #         return 501, "Usage: hashpass <cleartext password>"
    #     else:
    #         return 211, SHA256AES.create_passwd_entry(cleartext)

    # async def site_help(self, session: Session, _):
    ...
    # async def site_kick(self, session: Session, lookup):
    #     if session.uid != 0:
    ...
    # async def site_shutdown(self, session: Session, param):
    #     if session.uid != 0:
    #         return 550, "Not authorized."
    #     sync()
    #     await sleep_ms(1000)
    ...
    # async def site_uptime(self, session: Session, _):
    ...
    #     for s in self._session_list:
    #         login_time = FTPS.date_format(s.login_time)
    #         output.append(
